...
export const TRANSFORMS = {
  ...
  toIso: (v) => {
    ...
    const n = Number(v);
    const d = new Date(typeof v === 'number' || /^\d+$/.test(String(v)) ? n : v);
...
export function dedupeKey(item) {
  return String(item.jobId ?? item.documentId ?? item.url ?? item.title ?? JSON.stringify(item));
}
...
export function passesFilters(item, filters = {}) {
  if (filters.keywords?.length) {
    ...
    const hay = `${item.title ?? ''} ${item.department ?? ''} ${item.team ?? ''} ${item.description ?? ''}`;
